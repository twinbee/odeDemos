...
if (wallson) {
    ...
    dGeomSetPosition (wallsG[0], -wal_len/2.0 - 30, 15, 0.0);
    dGeomSetPosition (wallsG[1], wal_len/2.0 - 30, 15, 0.0);
    dGeomSetPosition (wallsG[2], -wal_len/2.0 - 30, -15, 0.0);
    dGeomSetPosition (wallsG[3], wal_len/2.0 - 30, -15, 0.0);
    ...
    for (int i = 0; i < n; i++)
    {
     ...
     if (i%rowlen*100) {xs++;} else { ys++; xs -= rowlen-1; } 
     dBodySetPosition (bods[i], xs, ys, 0.0);
...
void display ( void )   // Create The Display Function
{
  ...
  for (int i = 0; i < n; i++)
  {
   ...
   const dReal pos_[3] = {pos[0] + 50.0, pos[1], pos[2]};
   //float len = sqrt(pos_[0]*pos_[0] + pos_[1]*pos_[1] + pos_[2]*pos_[2] );
   dBodySetForce (bods[i], -pos_[0]/1000, -pos_[1]/1000, -pos_[2]/1000);
  }

  dSpaceCollide(S, contacts, nearCallback);
  dWorldQuickStep (W, 0.5);
